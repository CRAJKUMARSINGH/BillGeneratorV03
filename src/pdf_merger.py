...
class PDFMerger:
    ...
    def _create_fallback_pdf(self, doc_name: str, format_type: str, content: str = "") -> Optional[bytes]:
        """Create clean PDF when conversion tools are not available"""
        ...
        try:
            ...
            # Set up document
            pdf.setTitle(f"{doc_name.replace('_', ' ').title()}")
            ...
            clean_title = doc_name.replace('_', ' ').title()
            text_width = pdf.stringWidth(clean_title, "Helvetica-Bold", 18)
            pdf.drawString((width - text_width) / 2, height - 40*mm, clean_title)
            
            # Minimal content
            y_position = height - 80*mm
            pdf.setFont("Helvetica", 12)
            pdf.drawString(margin, y_position, "Document processed successfully.")
